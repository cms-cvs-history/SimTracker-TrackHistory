...
class testSVTagInfoValidationAnalyzer : public edm::EDAnalyzer
{

public:

    explicit testSVTagInfoValidationAnalyzer(const edm::ParameterSet&);
...
testSVTagInfoValidationAnalyzer::testSVTagInfoValidationAnalyzer(const edm::ParameterSet& config) : classifier_(config)
{
  //Initialize counters
    n_event = 0;
    rs_total_nall = 0; 
...
void testSVTagInfoValidationAnalyzer::analyze(const edm::Event& event, const edm::EventSetup& setup)
{
  ++n_event;
...
void testSVTagInfoValidationAnalyzer::bookRecoToSim(std::string const & prefix){
...
void testSVTagInfoValidationAnalyzer::bookSimToReco(std::string const & prefix){
  // Book pull histograms
...
void testSVTagInfoValidationAnalyzer::fillRecoToSim(std::string const & prefix, reco::Vertex const & vertex, TrackingVertexRef const & simVertex)
{
...
void testSVTagInfoValidationAnalyzer::fillSimToReco(std::string const & prefix, reco::VertexRef const & vertex, TrackingVertexRef const & simVertex)
{
...
void 
testSVTagInfoValidationAnalyzer::endJob() {
  ...
  std::cout << " ====== Total Number of Non-reconstructed Vertices: " << total_nmiss   << " ====== " << std::endl;
 
} 

#include "FWCore/PluginManager/interface/ModuleDef.h"
DEFINE_FWK_MODULE(testSVTagInfoValidationAnalyzer);
